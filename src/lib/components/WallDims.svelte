<script lang="ts">
	...
	const gap = $derived(thickness / 2 + 18 / scale);
	...
	const segs = $derived.by<Seg[]>(() => {
		const list: Seg[] = [];
		const ext = (from: Pt, to: Pt) =>
			list.push({ x1: from.x, y1: from.y, x2: to.x, y2: to.y, cls: 'ext' });
		const dim = (from: Pt, to: Pt) =>
			list.push({ x1: from.x, y1: from.y, x2: to.x, y2: to.y, cls: 'dim' });
		const cross = (p: Pt) =>
			list.push({
				x1: p.x - u.x * tick,
				y1: p.y - u.y * tick,
